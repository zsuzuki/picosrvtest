...
//
//
void LCD_Copy(POINT Xpoint, POINT Ypoint, LENGTH Width, LENGTH Height, const COLOR *Data)
{
	LENGTH scrW = sLCD_DIS.LCD_Dis_Column;
	LENGTH scrH = sLCD_DIS.LCD_Dis_Page;
	if (Xpoint > scrW || Ypoint > scrH)
		return;

	POINT Xr = Xpoint + Width;
	POINT Yb = Ypoint + Height;
	if (Xr <= scrW && Yb <= scrH)
	{
		// all data in screen
		LCD_SetWindow(Xpoint, Ypoint, Xr, Yb);

		DEV_Digital_Write(LCD_DC_PIN, 1);
		DEV_Digital_Write(LCD_CS_PIN, 0);
		for (uint32_t i = 0; i < Width * Height; i++)
		{
			SPI4W_Write_Byte(Data[i] >> 8);
			SPI4W_Write_Byte(Data[i] & 0XFF);
		}
		DEV_Digital_Write(LCD_CS_PIN, 1);
	}
	else
	{
		LCD_SetWindow(Xpoint, Ypoint, scrW, scrH);
		DEV_Digital_Write(LCD_DC_PIN, 1);
		DEV_Digital_Write(LCD_CS_PIN, 0);
		LENGTH w = scrW - Xpoint;
		for (LENGTH y = Ypoint; y < scrH; y++)
		{
			for (LENGTH x = 0; x < w; x++)
			{
				SPI4W_Write_Byte(Data[x] >> 8);
				SPI4W_Write_Byte(Data[x] & 0XFF);
			}
			Data += Width;
		}
		DEV_Digital_Write(LCD_CS_PIN, 1);
